...
with open("Lab5-6-2023.pdf", "rb") as file:
        contents = file.read()
        block_size = len(contents) // 8
        datablock = [contents[i:i+block_size] for i in range(0, len(contents), block_size)]

for i in range(len(datablock)):
        hash = hl.sha256(datablock[i]).hexdigest()
        blocks.append(hash)
...
for i in range(0, 2, 2):
        hash = block_2[i] + block_2[i+1]
        hash = hl.sha256(hash.encode()).hexdigest()
        MarkleRoot = hash
for block in range(8):
    print("Block Hashes:",blocks[block])

print("\n")

for block in range(4):
       print("Block_4 hashes: ", block_4[block])

print("\n")

for block in range(2):
       print("block_2 hashes: ", block_2[block])
print("\n")
print("Root hash: ", MarkleRoot)